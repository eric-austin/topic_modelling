...
def main():
    ...
    for network in os.listdir("./bbc_networks"):
        ...
        siwo_a = community_utils.read_siwo_comms(f"./bbc_siwo_comms/a_{network}")
        siwo_g = community_utils.read_siwo_comms(f"./bbc_siwo_comms/g_{network}")
        # filter small comms
        siwo_a = [comm for comm in siwo_a if len(comm) > 2]
        siwo_g = [comm for comm in siwo_g if len(comm) > 2]

        # load network
        nx_g = nx.read_weighted_edgelist(f"./bbc_networks/{network}")
